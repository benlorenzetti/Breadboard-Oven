...
\begin{flushleft}
	\begin{itemize}
		...
		\item 400 Watt nichrome heating element with door killswitch
...
Parameters of interest were the heating rate of the air in the chamber
and wattage consumed.
...
The fan is mounted in the back.
...
ceramic capacitors for electrical insulation. Structurally, the nichrome
...
The front facing user interface PCB mounted near the door and has no
voltages greater than 5V for safety. Everything on this front facing PCB
...
pushbuttons on the front facing board and with 6 corresponding low pass filters
...
Finally, there is a form A (normal open; active closed) kill switch
that is compressed when the door is closed. The current through this
switch directly drives the form A (normally open) safety relay on the controller board.
The kill switch circuit is shown in
\hyperref[killswitch-circuit]{figure \ref{killswitch-circuit}}.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\columnwidth]{Figures/killswitch-circuit.pdf}
	\caption{Kill Switch Safety Circuit}
	\label{killswitch-circuit}
\end{figure}
...
\begin{figure*}
	...
	\begin{circuitikz}[scale=.75]\draw
		(3,0) node [circ] (kn) {} to [R, l=$R_{1}$] (6,0)
		(3,.25) node [] {$K_{N}$}
		(2.5,-1) node [circ] (kp) {$\,K_{P}$} to (2.5,-1.5) node [ground] {}
		(6,0) to (6,1.5) to [R, l=$R_{2}$] (10,1.5) to (10,-.5)
...
The Seebeck voltage is the electromotive force that develops in a conductive loop
made of two different metal alloys. It is described by
\begin{equation}
...
should be
\begin{equation}
\frac{V_{\textrm{ADC(max)}}}{V_{\textrm{K(max)}}}=\frac{R_{2}}{R_{1}}
\end{equation}
\begin{equation*}
\frac{3.3\,mV}{20\,mV}=165
\end{equation*}
